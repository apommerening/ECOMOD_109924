# Functions for shot-noise regression. Based on Haebel et al. (2019). Updated on 09.09.2021.
# Stem-diameter potential
...
#----------------------------------#
timeSeries <- "giRegresPP.txt" # giRegresDF.txt, giRegresPP.txt, giRegresSP.txt, giRegresNS.txt, giRegresBE.txt, giRegresEucalypt4000.txt, giRegresEucalypt8055.txt, giRegresEucalypt8009.txt
...
nlsout <- nlrq(AGR ~ dpot(dbh, A, k, p), data = TreeLista, start = list(A = 0.03, k = 1.44, p = 0.03), # NS, BE, DF, FLNR, HK, PP, SP (tau = 0.975) # list(A = 0.18, k = 1.54, p = 0.01), for SP: list(A = 0.03, k = 1.44, p = 0.03)
...
maxG <- ceiling(max(TreeList$AGR, na.rm = T) * 10000) / 10000
# pdf(file = paste(outputFile, substr(timeSeries, 1, (nchar(timeSeries) - 4)), "_AGR_dbh_Potential.pdf", sep = ""))
par(mar = c(2, 2.2, 0.5, 0.5), mfrow = c(1, 1))
plot(TreeLista$dbh, TreeLista$AGR, las = 1, ylab = "", xlab = "", 
     cex = .9, col = "black", pch = 16, axes = FALSE, xlim = c(0, ceiling(max(TreeList$dbh, na.rm = T))), ylim = c(0, 4.0))#, ylim = c(0, 1)
...
plot(TreeLista$dbh, TreeLista$RGR, las = 1, ylab = "", xlab = "", 
...
loss.ML <- function(abdn, xTreeList, xparam, specCodes, fact, denom, relascope) {
  ...
  dev <- dnorm(xTreeList$AGR, mean = gi, sd = exp(abdn[length(abdn)]), log = TRUE)
  # is.na(dev) <- sapply(dev, is.infinite)
  if(denom == 0)
...
if(!relas)
  ...
  # abdn0 <- c(0.26, 0.34, 5.63, 0.85) # SP
  # abdn0 <- c(0.85, 0.26, 0.34, 5.63) # SP # These worked best!
...
par(lab = c(length(my.labels), 5, 7), mar = c(2, 3, 0.5, 0.5), mfrow = c(1, 1))
