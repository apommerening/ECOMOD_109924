...
nlsout <- nlrq(AGR ~ dpot(dbh, A, k, p), data = TreeLista, start = list(A = 0.03, k = 1.44, p = 0.03), # NS, BE, DF, FLNR, HK, PP, SP (tau = 0.975) list(A = 0.18, k = 1.54, p = 0.01)
...
loss.ML <- function(abdn, xTreeList, xparam, specCodes, fact, denom, relascope) {
  ...
  dev <- dnorm(xTreeList$RGR, mean = gi, sd = exp(abdn[length(abdn)]), log = TRUE)
  # is.na(dev) <- sapply(dev, is.infinite)
  if(denom == 0)
